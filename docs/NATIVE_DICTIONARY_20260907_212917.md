...
`tree`의 한국어판 원문에는 `(식물) 나무.`와 `나뭇가지 구조, 계보.`가 있으나 확보한 Kaikki raw는 뒤의 타동사1뜻만 포함했다. 한국어 글자가 존재한다는 빌더 검사는 이런 누락을 잡지 못한다. 수록량은 대표 뜻 충실성이나 원문 전체 추출의 보증이 아니다. 확인된 누락은 원문 근거를 갖춘 별도 보완 자료로 제공하며 원시 입력과 기존 뜻은 보존한다. 다른 단어의 같은 유형 누락 범위는 미확정이다.
